<script lang="ts">
  ...
  $effect(()=>{ const id=artist.id; reloadKey; let cancelled=false; loading=true; error=null;
    ...
    ]).then((results)=>{const failed=results.find((r)=>r.status==="rejected");if(!cancelled&&failed?.status==="rejected")error=store.handleError(failed.reason,false).message}).finally(()=>{if(!cancelled)loading=false});
