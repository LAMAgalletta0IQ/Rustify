<script lang="ts">
  ...
  import type { AlbumSummary, ArtistSummary, ConcertEvent, TrackSummary } from "../types";
  ...
  let concerts=$state<ConcertEvent[]>([]); let concertError=$state<string|null>(null);
  ...
  $effect(()=>{ const id=artist.id; reloadKey; let cancelled=false; loading=true; error=null;
    Promise.allSettled([
      ...
      api.getArtistTopTracks(id).then((v)=>{if(!cancelled)tracks=v}),
      ...
      api.getArtistConcerts(id).then((v)=>{if(!cancelled)concerts=v.events}).catch((e)=>{if(!cancelled)concertError=store.handleError(e,false).message}),
    ]).then((results)=>{const failed=results.slice(0,5).find((r)=>r.status==="rejected");if(!cancelled&&failed?.status==="rejected")error=store.handleError(failed.reason,false).message}).finally(()=>{if(!cancelled)loading=false});
...
<div class="artist">
  <div class="head" style={details.imageUrl?`--artist-image:url(${details.imageUrl})`:""}>
    ...
    <span class="identity"><span class="kind">Artist</span><h1 class="truncate">{details.name}</h1><small>{followed?"Following":"Not followed"} · Spotify does not expose monthly listeners to this integration</small></span>
  ...
  <section><h2>Tracks from recent releases</h2><p class="note">Spotify retired artist popularity rankings for this API; this is a deterministic playable sample, not a fabricated “Popular” chart.</p>{#if loading}<p class="muted">Loading tracks…</p>{:else if tracks.length}<TrackList tracks={showAllTracks?tracks:tracks.slice(0,5)} />{#if tracks.length>5}<button class="more" onclick={()=>showAllTracks=!showAllTracks}>{showAllTracks?"Show less":"Show more"}</button>{/if}{:else}<p class="muted">No playable tracks found.</p>{/if}</section>
  {#if likedTracks.length}<section><h2>Liked songs by {details.name}</h2><p class="note">Matched by Spotify artist ID across up to 500 saved tracks.</p><TrackList tracks={likedTracks}/></section>{/if}
  {#if concerts.length}<section><h2>On tour</h2><p class="note">Upcoming events listed by Spotify.</p><div class="concerts">{#each concerts as event (event.uri)}<article><time datetime={event.startDateIso??undefined}>{concertDate(event)}</time><div><strong>{event.title}</strong><span>{[event.venue,event.city].filter(Boolean).join(" · ")||"Venue TBA"}{event.isFestival?" · Festival":""}</span></div>{#if event.eventUrl}<button onclick={()=>openUrl(event.eventUrl!)}>View event</button>{/if}</article>{/each}</div></section>
  {:else if concertError}<p class="concert-note muted">Concerts are unavailable for this artist or region.</p>{/if}
